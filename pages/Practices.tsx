...
const Practices: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!window.location.hash || !window.location.hash.includes('#')) {
      ...
      // Handle initial hash from external links (e.g., from Home page)
      const hash = window.location.hash;
      const id = hash.split('#').pop(); // Get the last part after #
      if (id) {
        // Wait for elements to be in DOM and observer to be ready
        // Increased timeout to 500ms for better reliability with lazy loading
        setTimeout(() => {
    ...
    <div className="bg-white min-h-screen transition-colors duration-300">
      ...
      <div className="bg-surface-alt pt-32 pb-12 md:pt-40 md:pb-24 border-b border-gray-200 transition-colors duration-300">
        <div className="container mx-auto px-6 md:px-12">
          ...
          <h4 className="text-xxs font-bold tracking-[0.2em] text-primary uppercase mb-6 animate-fade-in">{t.services.title}</h4>
      ...
      <div className="container mx-auto px-6 md:px-12 py-12 md:py-32">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-24 relative">
          ...
          <div className="lg:hidden sticky top-24 z-30 mb-8 bg-white/95 backdrop-blur shadow-sm p-4 -mx-6 px-6 border-b border-gray-100">
            <label htmlFor="mobile-nav" className="text-xxs font-bold uppercase tracking-widest text-gray-500 mb-2 block">Jump to section</label>
