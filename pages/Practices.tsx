...
const Practices: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-primary min-h-screen transition-colors duration-300">
      ...
      <div className="bg-surface-alt dark:bg-[#1c1917] pt-32 pb-16 md:pt-40 md:pb-24 border-b border-gray-200 dark:border-white/5 transition-colors duration-300">
        <div className="container mx-auto px-6 md:px-12">
          <h4 className="text-[10px] font-bold tracking-[0.2em] text-secondary uppercase mb-6 animate-fade-in">{t.practices.capabilities}</h4>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-20">
          ...
          <div className="lg:col-span-3 hidden lg:block">
            <div className="sticky top-28">
              <h3 className="font-bold text-[10px] uppercase tracking-[0.2em] text-primary dark:text-white mb-6 border-b border-primary dark:border-white/20 pb-4">{t.practices.index}</h3>
              <ul className="space-y-3">
                {t.practicesList.map((practice) => (
                  <li key={practice.id} className="relative group/item">
                    <a 
                      href={`#${practice.id}`} 
                      className={`group flex items-center justify-between transition-colors text-xs font-medium py-1.5 ${
                    ...
                    {/* Tooltip */}
                    <div className="absolute left-full top-1/2 -translate-y-1/2 ml-6 w-64 p-5 bg-primary dark:bg-white text-white dark:text-primary text-xs font-light leading-relaxed rounded shadow-xl opacity-0 invisible group-hover/item:opacity-100 group-hover/item:visible transition-all duration-300 z-50 pointer-events-none translate-x-2 group-hover/item:translate-x-0">
          ...
          {/* Content with Spotlight Interaction - Reduced spacing */}
          <div className="lg:col-span-9 space-y-16" onMouseLeave={() => setHoveredPracticeId(null)}>
            {t.practicesList.map((practice, index) => (
              <div 
                key={practice.id} 
                id={practice.id} 
                className={`scroll-mt-32 group transition-all duration-500 ${
                  hoveredPracticeId !== null && hoveredPracticeId !== practice.id ? 'opacity-30 blur-[1px]' : 'opacity-100'
                }`}
              ...
              >
                <div className="flex flex-col md:flex-row gap-8 md:gap-16 border-t border-gray-200 dark:border-white/10 pt-16 group-first:border-t-0 group-first:pt-0">
                  <div className="md:w-1/4">
                    <span className={`text-4xl md:text-6xl font-serif transition-colors duration-500 block leading-none select-none ${
